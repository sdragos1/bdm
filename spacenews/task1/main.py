...
spark = SparkSession.builder.appName("SpaceNews - Task 1").master(
...
words_df = df.select(
    ...
    explode(
        split(
            regexp_replace(lower(col("title")), CLEAN_WORD_REGEX, " "),
            "\\s+"
        )
    ).alias("word")
...
word_counts_total.coalesce(1).write.mode("overwrite") \
    .csv(f"{HDFS_URI}/output/spacenews/title_word_count_total", header=False, sep=" ")
...
word_counts_per_day.coalesce(1).write.mode("overwrite") \
    .csv(f"{HDFS_URI}/output/spacenews/title_word_count_per_day", header=False, sep=" ")
